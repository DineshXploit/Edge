...
abstract class RepositoryModule {
    @Binds
    @Singleton
    abstract fun bindFitnessRepository(impl: FitnessRepositoryImpl): FitnessRepository
}
...
object DatabaseModule {
    ...
    fun database(@ApplicationContext context: Context): AppDatabase =
        Room.databaseBuilder(context, AppDatabase::class.java, "right_to_fitness_ai.db")
            .fallbackToDestructiveMigration(dropAllTables = true)
            .build()

    @Provides
    fun dao(db: AppDatabase): FitnessDao = db.dao()
}
